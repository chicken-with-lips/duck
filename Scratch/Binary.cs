...
public partial class ArrayFormatter : IMessagePackFormatter<Array>
{
    ...
    {
        var _serializationContext = new SerializationContext(false);
        var _graphSerializer = new GraphSerializer(_serializationContext);
        ...
        for (var index = 0; index < value.Length; index++) {
            ...
            if (obj is ISerializable ser) {
                ser.Serialize(_graphSerializer, _serializationContext);
            }

            MessagePackSerializer.Serialize(ref writer, obj, options);
        }

        var res = _graphSerializer.Close();

        MessagePackSerializer.Serialize(ref writer, res.Data.Length);
        MessagePackSerializer.Serialize(ref writer, res.Data);
        MessagePackSerializer.Serialize(ref writer, res.Index);
    }
    ...
    public Array Deserialize(ref MessagePackReader reader, MessagePackSerializerOptions options)
    {
        ...
        var len = MessagePackSerializer.Deserialize<int>(ref reader);
        var data = MessagePackSerializer.Deserialize<byte[]>(ref reader);
        var entries = MessagePackSerializer.Deserialize<IndexEntry[]>(ref reader);
...
public partial class GraphIndexEntryFormatter : IMessagePackFormatter<IndexEntry>
{
    public void Serialize(ref MessagePackWriter writer, IndexEntry value, MessagePackSerializerOptions options)
    {
        writer.WriteString(Encoding.UTF8.GetBytes(value.Name));
        writer.WriteUInt8((byte) value.Type);
        writer.WriteInt64(value.OffsetStart);
        writer.WriteInt64(value.OffsetEnd);
        writer.WriteString(Encoding.UTF8.GetBytes(value.ExplicitType));
    }
    ...
    public IndexEntry Deserialize(ref MessagePackReader reader, MessagePackSerializerOptions options)
    {
        ...
        entry.Type = (DataType)reader.ReadByte();
        entry.OffsetStart = reader.ReadInt64();
        entry.OffsetEnd = reader.ReadInt64();
        entry.ExplicitType = reader.ReadString();
