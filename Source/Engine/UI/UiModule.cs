...
public class UiModule : IUiModule, IInitializableModule, IPreTickableModule, IRenderableModule
{
    ...
    public UiModule(ILogModule logModule, IContentModule contentModule, IRendererModule rendererModule)
    {
    ...
    public void Render()
    {
        ...
        foreach (ref var root in Context.Roots) {
            ...
            var widthInEm = Measure.CalculateEmSizeFromPixels(view.Dimensions.X);
            var heightInEm = Measure.CalculateEmSizeFromPixels(view.Dimensions.Y);
        ...
        foreach (var primitive in primitives) {
            switch (primitive.Type) {
                case RenderPrimitiveType.Text:
                    ...
                    var xOffset = primitive.Position.X;
                    var yOffset = primitive.Position.Y;
                    var scale = 0.5f;

                    yOffset = primitive.Position.Y + (font.Ascender * scale);
                ...
                case RenderPrimitiveType.Box:
                    ...
                    var boxTopLeft = new Vector3D<float>(primitive.Position.X, primitive.Position.Y, 0);
                    var boxTopRight = new Vector3D<float>(primitive.Position.X + primitive.Dimensions.X, primitive.Position.Y, 0);
                    var boxBottomRight = new Vector3D<float>(primitive.Position.X + primitive.Dimensions.X, primitive.Position.Y + primitive.Dimensions.Y, 0);
                    var boxBottomLeft = new Vector3D<float>(primitive.Position.X, primitive.Position.Y + primitive.Dimensions.Y, 0);
        ...
        if (boxIndexCount > 0) {
            _boxIndexBuffer.SetData(0, boxIndices);
            _boxVertexBuffer.SetData(0, boxVertices);
        }

        if (textIndexCount > 0) {
            _textIndexBuffer.SetData(0, textIndices);
            _textVertexBuffer.SetData(0, textVertices);
        }
