...
public class CharacterControllerIntegration
{
    ...
    public void Allocate(int entityId, ref CharacterControllerComponent characterControllerComponent, PhysicsBodyComponent physicsBodyComponent)
    {
        ...
        if (bodyHandle.Value >= _bodyHandleToCharacterIndex.Length) {
            ResizeBodyHandleCapacity(System.Math.Max(bodyHandle.Value + 1, _bodyHandleToCharacterIndex.Length * 2));
        }
    ...
    bool TryReportContacts<TManifold>(CollidableReference characterCollidable, CollidableReference supportCollidable, CollidablePair pair, ref TManifold manifold, int workerIndex) where TManifold : struct, IContactManifold<TManifold>
    {
        if (characterCollidable.Mobility == CollidableMobility.Dynamic && characterCollidable.BodyHandle.Value < _bodyHandleToCharacterIndex.Length) {
            ...
            if (characterIndex >= 0) {
                ...
                ref RigidPose pose = ref set.SolverStates[bodyLocation.Index].Motion.Pose;

                QuaternionEx.Transform(character.LocalUp.ToSystem(), pose.Orientation, out var up);
    ...
    private unsafe void ExpandBoundingBoxes(int start, int count)
    {
        ...
        for (int i = start; i < end; ++i) {
            ...
            if (characterBody.Awake) {
                ...
                QuaternionEx.Transform(character.LocalUp.ToSystem(), characterBody.Pose.Orientation, out var characterUp);
    ...
    private void PrepareForContacts(float dt, IThreadDispatcher? threadDispatcher = null)
    {
        ...
        if (threadCount == 1 || _characters.Count < 256) {
            ...
            int jobCount = System.Math.Min(_characters.Count, threadCount);
        ...
        for (int characterIndex = start; characterIndex < exclusiveEnd; ++characterIndex) {
            ...
            if (bodyLocation.SetIndex == 0) {
                ...
                if (supportCandidate.Depth > float.MinValue && character.TryJump) {
                    QuaternionEx.Transform(character.LocalUp.ToSystem(), _simulation.Bodies.ActiveSet.SolverStates[bodyLocation.Index].Motion.Pose.Orientation, out var characterUp);
                    ...
                    float characterUpVelocity = Vector3.Dot(_simulation.Bodies.ActiveSet.SolverStates[bodyLocation.Index].Motion.Velocity.Linear, characterUp);
                    ...
                    if (character.Support.Mobility != CollidableMobility.Static) {
                        ...
                        ref BodyVelocity supportVelocity = ref _simulation.Bodies.ActiveSet.SolverStates[supportingBodyLocation.Index].Motion.Velocity;
                    ...
                    QuaternionEx.Transform(character.LocalUp.ToSystem(), _simulation.Bodies.ActiveSet.SolverStates[bodyLocation.Index].Motion.Pose.Orientation, out var up);

                    var rayDistance = Vector3.Dot(character.ViewDirection.ToSystem(), surfaceBasis.Y);
                    ...
                    surfaceBasis.Z = up * (rayDistance / rayVelocity) - character.ViewDirection.ToSystem();
                    ...
                    if (supportCandidate.Support.Mobility != CollidableMobility.Static) {
                        ...
                        DynamicCharacterMotionConstraint motionConstraint = new DynamicCharacterMotionConstraint {
                            ...
                            TargetVelocity = character.TargetVelocity.ToSystem(),
                            Depth = supportCandidate.Depth
                        };

                        if (character.Supported && !shouldRemove) {
                            // Already exists, update it.
                            _simulation.Solver.ApplyDescriptionWithoutWaking(character.MotionConstraintHandle, motionConstraint);
                        } else {
    ...
    private void AnalyzeContacts(float dt, IThreadDispatcher? threadDispatcher)
    {
        ...
        if (threadDispatcher == null) {
            ...
            _analysisJobCount = System.Math.Min(_characters.Count, threadDispatcher.ThreadCount * 4);
        ...
        if (_analyzeContactsWorkerCaches.Allocated) {
            ...
            for (int threadIndex = 0; threadIndex < _analyzeContactsWorkerCaches.Length; ++threadIndex) {
                ...
                for (int i = 0; i < workerCache.StaticConstraintsToAdd.Count; ++i) {
                    ...
                    character.MotionConstraintHandle = _simulation.Solver.Add(character.BodyHandle, pendingConstraint.Description);
                }
                ...
                for (int i = 0; i < workerCache.DynamicConstraintsToAdd.Count; ++i) {
                    ...
                    character.MotionConstraintHandle = _simulation.Solver.Add(character.BodyHandle, character.Support.BodyHandle, pendingConstraint.Description);
                }
                ...
                for (int i = 0; i < workerCache.Jumps.Count; ++i) {
                    ...
                    if (jump.SupportBodyIndex >= 0) {
                        BodyReference.ApplyImpulse(_simulation.Bodies.ActiveSet, jump.SupportBodyIndex, jump.CharacterVelocityChange / -activeSet.SolverStates[jump.CharacterBodyIndex].Inertia.Local.InverseMass, jump.SupportImpulseOffset);
                    }
                }
    ...
    public void Resize(int characterCapacity, int bodyHandleCapacity)
    {
        ...
        int targetHandleCapacity = BufferPool.GetCapacityForCount<int>(System.Math.Max(lastOccupiedIndex + 1, bodyHandleCapacity));

        if (targetHandleCapacity != _bodyHandleToCharacterIndex.Length) {
            ResizeBodyHandleCapacity(targetHandleCapacity);
        }

        int targetCharacterCapacity = BufferPool.GetCapacityForCount<int>(System.Math.Max(_characters.Count, characterCapacity));
