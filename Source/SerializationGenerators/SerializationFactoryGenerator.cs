    ...
    private static string GenerateCode(SourceProductionContext context, ImmutableArray<ITypeSymbol?> types)
    {
        ...
        foreach (var type in types.Distinct(SymbolEqualityComparer.Default)
            ...
            if (type.IsGenericType) {
                ...
                // serializeBuilder.AppendLine($@"case ""{fqdn}"": {type.ContainingNamespace}.{type.ConstructUnboundGenericType().Name}Serializer.Serialize(({fqdn}) value, graphWriter, context); break;");
            } else {
                deserializeBuilder.AppendLine($@"""{fqdn}"" => {fqdn}Serializer.Deserialize(reader),");
                serializeBuilder.AppendLine($@"case ""{fqdn}"": {fqdn}Serializer.Serialize(({fqdn}) value, writer); break;");
            }
        }
