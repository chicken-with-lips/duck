...
public class SerializerGenerator : IIncrementalGenerator
{
    ...
    private static string GenerateSerializerMethod(INamedTypeSymbol type)
    {
        ...
        foreach (var member in type.GetMembers()) {
            ...
            if (Util.IsSupportedContainerType(fieldSymbol.Type)) {
                sb.AppendLine($@"writer.Write(""{symbolName}"", value.{symbolName}.ToArray(), ""{fieldSymbol.Type.ContainingNamespace}.{fieldSymbol.Type.Name}"");");
            } else {
                sb.AppendLine($@"writer.Write(""{symbolName}"", value.{symbolName});");
            }
        }
    ...
    private static string GenerateDeserializerMethod(INamedTypeSymbol type)
    {
        ...
        return $$"""
                 public static {{typeName}} Deserialize{{genericParameters}}(GraphReader reader) {{genericConstraints}}
                 {
                     var ret = new {{typeName}}();
                 
                     {{addToContextStr}}

                     foreach (var entry in reader.Index) {
                         switch(entry.Name) {
        ...
        foreach (var member in type.GetMembers()) {
            ...
            if (Util.IsSupportedContainerType(fieldSymbol.Type)) {
                deserializeMemberBuilder.AppendLine($@"case ""{symbolName}"":
                     ret.{symbolName} = reader.ReadObjectList<{fieldSymbol.Type.ToDisplayString()}, {fieldTypeSymbol.TypeArguments[0].ToDisplayString()}>(
                ...
                if (doesImplementISerializable) {
                    deserializeMemberBuilder.AppendLine($@"case ""{symbolName}"":
                    {symbolName} = reader.ReadObjectReference<{fieldSymbol.Type.ToDisplayString()}>(
                    ...
                    if (fieldTypeSymbol.IsGenericType) {
                        ...
                        deserializeMemberBuilder.AppendLine(
                            $@"case ""{symbolName}"": ret.{symbolName} = reader.Reader.{methodName}<{argString}>(entry.OffsetStart); break;");
                    } else {
                        deserializeMemberBuilder.AppendLine(
                            $@"case ""{symbolName}"": ret.{symbolName} = reader.Reader.{methodName}(entry.OffsetStart); break;");
                    }
                }
            }
        }
